...
function calculateWinner(squares) {
  ...
  for (let col = 0; col < BOARD_SIZE; col++) {
    for (let row = 0; row <= BOARD_SIZE - WIN_LENGTH; row++) {
      const line = Array.from({ length: WIN_LENGTH }, (_, i) => (row + i) * BOARD_SIZE + col);
      if (checkLine(squares, line)) {
        return { winner: squares[line[0]], line };
      }
    }
  ...
  for (let row = 0; row <= BOARD_SIZE - WIN_LENGTH; row++) {
    for (let col = 0; col <= BOARD_SIZE - WIN_LENGTH; col++) {
      const line = Array.from({ length: WIN_LENGTH }, (_, i) => (row + i) * BOARD_SIZE + (col + i));
      if (checkLine(squares, line)) {
        return { winner: squares[line[0]], line };
      }
    }
    ...
    <div className="min-h-screen bg-gray-50 text-gray-900 p-8">
      ...
      <div className="flex flex-row gap-10 justify-center items-start max-w-7xl mx-auto">
        <div className="flex-shrink-0">
          {/* Game status */}
          <div className="text-2xl font-bold text-center text-blue-600 mb-4">{status}</div>
          ...
          <Board
            ...
            disabled={winner !== null || currentSquares.every(square => square !== null)}
          />
        </div>
